    ...
    <div class="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-5">
        ...
        @foreach ($cards as $i => $c)
            <div class="stat-card surface-hover animate-rise" style="--stat-accent: {{ $c['accent'] }}; animation-delay: {{ $i * 60 }}ms">
                <p class="text-[11px] font-semibold uppercase tracking-[0.12em] text-stone-500">{{ $c['label'] }}</p>
    ...
    <section class="surface overflow-hidden">
        ...
        @if ($recent->isEmpty())
            <div class="empty-state">
                <p class="text-sm text-stone-400">Henüz rezervasyon bulunmuyor.</p>
            </div>
        @else
                            ...
                            <tr>
                                <td>
                                    <a href="{{ route('admin.reservations.show', $r) }}" class="block hover:text-teal-800">
                                        ...
                                        <p class="text-xs text-stone-400">{{ $r->user->maskedTcNo() }}</p>
                                    </a>
                                </td>
