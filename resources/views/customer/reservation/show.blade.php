    ...
    <div class="mx-auto max-w-2xl">
        ...
        @if (session('success') && $reservation->status === 'pending')
            <div class="surface mt-6 mb-6 overflow-hidden animate-rise">
                <div class="bg-gradient-to-br from-teal-50 to-white px-6 py-10 text-center">
                    ...
                    <p class="mx-auto mt-2 max-w-sm text-sm text-slate-500">Admin onayı bekleniyor. İnceleme tamamlandığında bu sayfadan durumu takip edebilirsiniz.</p>
                </div>
            </div>
        @else
        ...
        <div class="surface overflow-hidden">
            ...
            <div class="divide-y divide-slate-100/80">
                ...
                @if ($reservation->note)
                    <div class="flex justify-between gap-6 px-6 py-3.5 text-sm"><span class="text-slate-500">Notunuz</span><span class="max-w-xs text-right font-medium text-navy-900">{{ $reservation->note }}</span></div>
                @endif
                @if ($reservation->admin_note && in_array($reservation->status, ['rejected', 'approved']))
                    <div class="flex justify-between gap-6 px-6 py-3.5 text-sm"><span class="text-slate-500">Yönetici notu</span><span class="max-w-xs text-right font-medium text-navy-900">{{ $reservation->admin_note }}</span></div>
                @endif
        ...
        @if ($reservation->status === 'approved')
            <div class="surface mt-6 overflow-hidden text-center">
                <div class="bg-gradient-to-br from-teal-50 via-white to-sand-50 px-6 py-8">
                    ...
                    <p class="mx-auto mt-1.5 max-w-sm text-sm text-slate-500">Yerinizi kesinleştirmek için ödemenizi tamamlayabilirsiniz.</p>
        ...
        @elseif ($reservation->status === 'paid' && $reservation->payment)
            <div class="surface mt-6 px-6 py-5">
                ...
                <p class="mt-2 text-sm text-slate-600">Referans No: <span class="font-semibold text-navy-800">{{ $reservation->payment->reference_no }}</span></p>
                <p class="text-sm text-slate-600">Ödeme tarihi: {{ $reservation->payment->paid_at->translatedFormat('d M Y H:i') }}</p>
            </div>
